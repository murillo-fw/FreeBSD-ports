...
function e2gm($field){
	...
	if ($filter != "") {
		$field = preg_replace("@($filter)@i",
		    "<span><font color='red'>$1</font></span>", $field);
	}
	return $field;
}
...
if ($_POST) {
	$filter = preg_replace('/(@|!|>|<)/', "",
	    htmlspecialchars($_POST['strfilter']));
	$program = strtolower($_POST['program']);
	$listr = "class='listr' style='font-family: Consolas, Lucida Console," .
	    " monospace;'";
	switch ($program) {
	case 'access':
		// Define log file
		...
		case 1:
			show_tds(array("","Date", "IP", "Url", "Response",
			    "User", "Group", "Reason"));
			foreach ($logarr as $logent) {
				//split log
				if (!preg_match("/(\S+\s+\S+) (\S+) (\S+) (\S+) (.*) (GET|OPTIONS|POST|CONNECT|-) \d+ \d+ (.*) \d (\d+) \S+ \S+ (\S+)/",
				    $logent, $logline)) {
					continue;
				}
		...
		case 3:
			...
			foreach ($logarr as $logent) {
				...
				echo "<td class='listlr' nowrap='nowrap'>" .
				    e2gm("{$logline[0]} {$logline[1]}") . "</td>\n";
				echo "<td $listr>" . e2gm($logline[8]) . "</td>\n";
				echo "<td $listr>" . e2gm($logline[3]) . "</td>\n";
				echo "<td $listr'>" . e2gm($logline[4]) . "</td>\n";
				echo "<td $listr title='{$logline[7]}'width='*'>" .
				    e2gm(preg_replace("/(\?|;).*/","",$logline[7])) . "</td>\n";
				echo "</tr>\n";
			}
			break;
		case 4:
			show_tds(array("","IP", "Method", "Url", "Response",
			    "Reason", "List Category", "Group"));
			foreach ($logarr as $logent) {
				...
				if ($_REQUEST['error'] == 'detailed') {
					echo "<td $listr title='{$logline[3]}' width='*'>" .
					    e2gm($url) . "</td>\n";
				} else {
					echo "<td $listr title='{$logline[3]}' width='*'>" .
					    e2gm(preg_replace("/(\?|;).*/","",$url)) . "</td>\n";
